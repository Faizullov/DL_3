...
def prior_boxes(cfg):
    ...
    for k, feature_map in enumerate(feature_maps):
        for i, j in product(range(feature_map[0]), range(feature_map[1])):   # возвращаем декартово произведение
            # unit center x,y
            cx = (j + 0.5) / feature_map[1]     #find center
            cy = (i + 0.5) / feature_map[0]
            # aspect_ratio: 1
            # rel size: min_size
            s_k = min_sizes[k]                  #min size
            prior_box_s += [cx, cy, s_k, s_k]   # create first prior box
            # aspect_ratio: 1
            # rel size: sqrt(s_k * s_(k+1))
            s_k_prime = sqrt(min_sizes[k] * (max_sizes[k]))  
            prior_box_s += [cx, cy, s_k_prime, s_k_prime]    # create second prior box
            # rest of aspect ratios
            for ar in aspect_ratios[k]:
                prior_box_s += [cx, cy, s_k*sqrt(ar), s_k/sqrt(ar)]    # add another boxes
                prior_box_s += [cx, cy, s_k/sqrt(ar), s_k*sqrt(ar)]
    # back to torch land
    prior_box_s = torch.Tensor(prior_box_s).view(-1, 4)
    if clip:
        prior_box_s.clamp_(max=1, min=0)     # обрезка тензора, чтобы значения были от 0 до 1
    return prior_box_s
...
def match(threshold, truths, priors, variances, labels):
    ...
    """
    # jaccard index
    overlaps = jaccard(truths, point_form(priors))   # convert priosr to point form and jaccard
    # (Bipartite Matching)
    ...
    return loc, conf   # [num_priors,4] encoded offsets to learn # [num_priors] top class label for each prior нашли как локализовывать + где объекты
...
def decode(loc, priors, variances):
    ...
    boxes[:, :2] -= boxes[:, 2:] / 2
    boxes[:, 2:] += boxes[:, :2]
    return boxes       # получили скорректированные результаты 
